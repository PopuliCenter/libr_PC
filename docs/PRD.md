...
| P2 ✅* | **Pencarian semantik & tanya-jawab koleksi (RAG)** — pengguna bertanya → jawaban dengan rujukan halaman, dibaca di protected reader. **Terimplementasi**: modul `rag/` — ekstraksi teks per-halaman (MuPDF) → `document_chunks` (auto-index saat upload); retrieval leksikal (INTERNAL dikecualikan); `POST /rag/ask` → jawaban ber-rujukan halaman (Claude bila `ANTHROPIC_API_KEY` ada, selain itu ekstraktif). Halaman `/tanya` dgn tautan ke reader. **\*Catatan:** retrieval masih leksikal (bukan embedding vektor) & ekstraksi butuh lapisan teks PDF (scan tanpa OCR belum terindeks) — keduanya jalur peningkatan. |
