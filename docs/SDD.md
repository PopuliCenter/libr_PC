...
- Tipe akses: `OPEN` (publik), `MEMBER` (login), `LOAN` (harus pinjam), `INTERNAL` (peneliti internal — PRD P1).
- **Anotasi pribadi (PRD P5):** modul `annotations/` — entity `annotations` (userId, documentId, pageNo, note) + endpoint `/me/annotations` (member). Panel "Catatan" di protected reader: tambah catatan pada halaman aktif, lompat ke halaman catatan, edit/hapus. **Disimpan per akun di DB, bukan di berkas** → watermark/DRM master tak tersentuh (justru tujuan desainnya). Setiap operasi terikat pemilik (privasi antar-anggota, uji 403 lintas-pengguna); catatan kosong/whitespace ditolak (Transform trim + IsNotEmpty); membuat anotasi butuh hak baca koleksi (INTERNAL tetap dijaga).
- **RAG / tanya-jawab koleksi (PRD P2):** modul `rag/`. **Indexing** — saat PDF master diunggah, event `document.digitized` memicu ekstraksi teks per halaman (MuPDF `toStructuredText().asText()`) → tabel `document_chunks` (`documentId`, `pageNo`, `chunkIndex`, `text`; halaman panjang dipecah pada batas kalimat/paragraf). **Retrieval** — leksikal BM25-lite: tokenisasi + stopword, kandidat via `LIKE` (cap 400), skor cakupan-istilah + frekuensi, cuplikan di sekitar kecocokan; **koleksi INTERNAL tak pernah diambil** (hanya PUBLISHED non-INTERNAL). **Answer** — `RagService.ask`: bila `ANTHROPIC_API_KEY` ada → Claude mensintesis jawaban *grounded* ke kutipan bernomor (wajib rujuk `[n]`, tanpa mengarang); bila tidak → mode ekstraktif (kutipan teratas). Respons memuat `citations` (judul, slug, `pageNo`, snippet) → halaman `/tanya` menautkan ke protected reader. Rujukan halaman = janji PRD "jawaban dengan rujukan halaman, dibaca di protected reader". **Batas jujur:** retrieval leksikal (bukan vektor semantik) & bergantung lapisan teks PDF (scan butuh OCR) — arsitektur `RetrievalService` sengaja dibuat agar retriever embedding dapat menggantikan.
- **Rekomendasi bacaan (PRD P3):** modul `recommendations/` — `GET /documents/:id/recommendations` (@Public, cache 5 mnt). Collaborative filtering item-item: pembaca koleksi sumber → koleksi lain yang mereka baca (co-read), diurutkan frekuensi. **Cold-start** ditangani berlapis: co-read → kategori sama (terpopuler lalu terbaru) → terbitan terbaru. Koleksi INTERNAL & koleksi sumber tak pernah muncul (bahkan pada fallback recency). Agregasi di aplikasi (portabel). Ditampilkan di halaman detail dgn label dasar rekomendasi.
- **Koleksi internal role-based (PRD P1):** flag `user.isInternal` + helper `hasInternalAccess(user)` (internal, atau pustakawan/superadmin). `INTERNAL` disembunyikan di **semua** jalur ekspos: `search` (filter), detail (`404`, tak membocorkan keberadaan), reader (`Forbidden`), **OAI `ListRecords`/`GetRecord`**, **RSS**, **widget** — kritis agar arsip internal tak ter-harvest ke jejaring nasional. Guard JWT global diubah jadi **auth opsional** pada rute `@Public` (melampirkan user bila token valid, tanpa menolak) → satu katalog untuk publik & internal. Karena auth berbasis bearer/localStorage (bukan cookie), halaman detail (server component) yang gagal fetch anonim menyerahkan pengambilan ke klien (dgn token); metadata server tetap generik agar judul internal tak bocor ke crawler. Superadmin menandai anggota internal via `PATCH /admin/users/:id` (`isInternal`).
- **Multibahasa ID/EN (PRD P4):** kolom `titleEn`/`abstractEn` (opsional) → katalog & metadata dwibahasa. Frontend: `LanguageProvider` (preferensi `localStorage`, set `<html lang>`) + `lib/i18n` (kamus permukaan publik) + pengalih ID/EN di header. Konten judul/abstrak terlokalisasi dgn **fallback ke ID** bila EN kosong (`docTitle`/`docAbstract`); label UI katalog/kartu/detail via `t()`. Pencarian `LIKE` mencakup `titleEn`/`abstractEn`. OAI-PMH memancarkan `dc:title`/`dc:description` ganda ber-`xml:lang` (id + en) untuk harvest internasional. Alat internal (admin form, reader) sengaja tetap ID; i18n UI menyeluruh (semua halaman) adalah lanjutan.
- **DOI (PRD I10):** kolom `documents.doi` (nullable). Input admin dinormalisasi (`https://doi.org/…`/`doi:` → DOI telanjang) & divalidasi pola `^10\.\d{2,9}/\S+$`. DOI disebar ke **semua** permukaan sitasi/harvest: tautan `https://doi.org/…` di halaman detail, meta Google Scholar `citation_doi`, sitasi APA/Chicago/BibTeX (`doi = {…}`), OAI-PMH `dc:identifier`, & RSS `dc:identifier`. Registrasi/deposit ke Crossref/DataCite **belum** (butuh keanggotaan + kredensial) — DOI dimasukkan manual.
